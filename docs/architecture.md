...
`priority` 按路由优先级选择，并在请求尚未发出时跳过额度耗尽、固定份额为 0 或凭据不可用的 Plan；`balanced` 在固定分配模式按“成员已用额度 / 成员份额”比较，在共享模式按账号总额度使用比例比较，优先级用于比例相同的排序。上游明确以 `429` 或 `529` 拒绝执行时最多切换 3 个账号；连接错误、超时和其他响应不重试，避免重复执行。
...
每次成功响应后，ShareSub 更新账号窗口的已用额度快照。同一窗口只接受更高的已用百分比；较低的乱序观测不会覆盖现有值。系统仍保留按响应观测生成的额度增量事件用于历史诊断，但固定份额限制不使用该归因结果。

百分比以一个百分点的百万分之一为单位，使用整数保存，避免浮点数累计误差。例如 25% 保存为 `25000000`。固定分配模式分别在 5 小时和 7 天有效窗口内，按“账号当前已用额度 × 成员窗口账号费用 ÷ 全部成员窗口账号费用”估算成员已用额度；任一窗口达到其份额后不再参与选路。共享模式不执行个人额度限制；账号任一有效窗口达到 100% 后不再参与选路。
...
Plan 详情与固定份额网关限制使用同一口径：按当前账号窗口内网关记录的 `estimated_cost_micros` 汇总费用占比，再乘以账号窗口当前已用额度。成员 5h/7d 显示值因此表示其估算已用账号额度，而不是成员间合计为 100% 的相对成本占比。
