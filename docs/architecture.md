...
Plan 详情中的成员 5h/7d 占比不使用上述配额增量归因。它按当前账号窗口内网关记录的 `estimated_cost_micros` 汇总，用成员账号费用除以全体当前成员账号费用得到占比。百分比以最大余数法分配微单位，确保有费用的窗口合计精确为 100%。该指标用于展示成员相对成本；固定份额模式的网关限额仍使用配额窗口累计值。
