...
export function createPlanManagementAPI(adminMode = false) {
  return adminMode ? {
    ...
    quotaResetVote: async (_id: string) => ({ vote: null }),
    createQuotaResetVote: async (_id: string) => { throw new Error('管理员模式不参与成员投票') },
    castQuotaResetVote: async (_planID: string, _voteID: string, _choice: 'support' | 'oppose') => { throw new Error('管理员模式不参与成员投票') },
    ...
    convertPlanToFixed: adminAPI.adminConvertPlanToFixed,
    ...
    quotaResetVote: api.quotaResetVote,
    createQuotaResetVote: api.createQuotaResetVote,
    castQuotaResetVote: api.castQuotaResetVote,
    invite: api.invite,
    ...
    convertPlanToFixed: api.convertPlanToFixed,
