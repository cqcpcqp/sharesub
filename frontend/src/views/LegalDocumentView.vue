...
interface LegalDocument { title: string; englishTitle: string; version: string; intro: string; sections: readonly LegalSection[] }
const content: Record<Exclude<PublicPageID, 'home'>, LegalDocument> = {
  terms: {
    title: '用户协议', englishTitle: 'TERMS OF SERVICE', version: agreementVersions.terms,
    intro: '欢迎使用 ShareSub。注册、访问或使用本服务前，请认真阅读本协议以及与本协议一并适用的隐私政策和可接受使用规范。',
    sections: [
      { title: '服务说明', paragraphs: ['ShareSub 是面向 OpenAI Codex 使用场景的账号共享管理工具，提供账号接入、Plan 管理、成员协作、额度归属、API Key 管理和请求转发等功能。ShareSub 不是 OpenAI 官方产品，与 OpenAI 无隶属、授权或代理关系。', '平台不销售 OpenAI 账号或上游服务本身。你对上游账号和服务的使用仍受 OpenAI 的条款、政策和限制约束。'] },
      ...
      { title: '账号接入与授权', paragraphs: ['你只能接入自己拥有或已获得明确授权使用的 OpenAI 账号。创建 Plan、邀请成员或公开席位前，你应确认自己有权授予相应访问。', '房主负责决定成员、额度和共享范围；成员应仅在获准范围内使用。房主与成员之间的授权争议由相关用户自行解决，平台会在必要时采取限制措施以保护系统和其他用户。'] },
  ...
  privacy: {
    title: '隐私政策', englishTitle: 'PRIVACY POLICY', version: agreementVersions.privacy,
    intro: '本政策说明 ShareSub 在提供服务过程中处理哪些信息、为什么处理以及如何保存这些信息。我们不会把尚未实现的能力写成承诺。',
    sections: [
      { title: '我们处理的信息', paragraphs: ['为创建和维护账户，我们处理用户名、邮箱、密码哈希、头像、账户状态、登录会话和创建时间。密码不会以明文保存。'], items: ['你主动配置的 OpenAI 账号名称、备注、代理与限速策略', '经加密保存的 OpenAI OAuth 凭据及其有效期', 'Plan、成员、邀请、申请和 API Key 配置', '请求状态、Token 用量、模型、延迟、额度归属和聚合性能指标', '安全与管理操作形成的审计记录和通知'] },
      ...
      { title: '共享与可见范围', paragraphs: ['Plan 成员可以查看该 Plan 所绑定账号的配置、账号额度和成员用量。公开 Plan 会向已登录用户展示 Plan 名称与描述、房主用户名与头像、账号绑定状态、账号订阅类型（如已绑定）、成员数和可用席位。房主应在公开 Plan 前确认这些信息适合展示。', '我们不会因为营销目的出售个人信息。为完成请求，必要数据会被传输至用户所选择的上游服务；上游如何处理数据受其自身政策约束。'] },
      { title: '保存期限', paragraphs: ['会话在到期或退出后失效。系统按照部署配置清理过期资源：网关指标和额度归因事件默认保留 90 天，审计记录默认保留 365 天，已读通知及已结束的邀请、申请和撤销 Key 默认保留 90 天；实际部署可以在系统允许范围内调整保留期。未读通知不会自动删除。', '账号和有效业务数据在提供服务所需期间保存，或按照适用法律和有效请求进行处理。'] },
  ...
  'acceptable-use': {
    title: '可接受使用规范', englishTitle: 'ACCEPTABLE USE POLICY', version: agreementVersions.acceptableUse,
