...
const content: Record<Exclude<PublicPageID, 'home'>, LegalDocument> = {
  terms: {
    title: '用户协议', englishTitle: 'TERMS OF SERVICE', version: agreementVersions.terms, effectiveDate: '2026 年 8 月 17 日',
    ...
    sections: [
      ...
      { title: 'Plan 与 API Key', paragraphs: ['固定分配模式按照成员份额管理额度，共享模式由成员共享账号总额度。房主可以将共享模式单向转换为固定分配；转换前当前额度窗口中的成员用量继续计入转换后的个人额度，转换后不能改回共享模式。具体可用性同时受到上游额度、并发、RPM 和服务状态影响。', 'API Key 归创建它的用户所有。你不得泄露、转售或允许无权人员使用密钥，并应在不再使用时及时撤销。'] },
