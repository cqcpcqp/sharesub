...
const content: Record<Exclude<PublicPageID, 'home'>, LegalDocument> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: '共享与可见范围', paragraphs: ['Plan 成员可以查看该 Plan 所绑定账号的配置、账号额度和成员用量。公开 Plan 会向已登录用户展示 Plan 名称、房主用户名与头像、账号订阅类型、成员数和可用席位。房主应在公开 Plan 前确认这些信息适合展示。', '我们不会因为营销目的出售个人信息。为完成请求，必要数据会被传输至用户所选择的上游服务；上游如何处理数据受其自身政策约束。'] },
