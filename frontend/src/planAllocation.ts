...
export function planApprovedPublicMemberCount(detail: PlanDetail): number {
...
export function planAvailablePublicSlotCount(detail: PlanDetail, publicSlotCount = detail.plan.public_slots): number {
  return Math.max(0, publicSlotCount - planApprovedPublicMemberCount(detail))
}
...
export function planPublicationShareBasisPoints(
  ...
  const availablePublicSlots = planAvailablePublicSlotCount(detail, publicSlotCount)
