...
const nextAction = computed(() => {
  ...
  if (path.value === 'member' && props.plans.length === 0) return { title: '加入一个 Plan', description: '邀请链接会在登录后自动接受，也可以从大厅申请公开招募名额。', primaryLabel: '探索大厅', secondaryLabel: '', icon: Compass, action: 'lobby' as const }
