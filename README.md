...
ShareSub 是一个仅支持 OpenAI Codex 的账号共享平台。房主接入自己的 OpenAI 账号并创建 Plan，可以选择按成员固定分配份额，也可以让所有成员共享账号总额度。Plan 可通过一次性链接邀请成员，也可以预留公开席位后发布到大厅。用户申请公开席位后，由房主批准或拒绝。
...
- 固定分配 Plan 中，所有有效份额与待接受邀请的份额之和不能超过 100%；共享使用 Plan 不设置个人份额或个人额度上限。
- 公开 Plan 上架时必须预留席位；固定分配模式还需设置每席份额，共享模式的每席份额固定为 `0`。
- 额度同时按 Codex 5 小时窗口和 7 天窗口执行。
- 登录后默认进入个人仪表盘，可查看今日/累计 Token、RPM/TPM、平均响应、成功率和最近 24 小时趋势。
- Plan 成员可以查看账号额度、每位成员的额度消耗、24 小时聚合性能和最近 7 天请求指标；系统不保存请求正文。
...
1. 使用唯一用户名、邮箱和密码注册并登录 ShareSub。用户名可在“个人设置”中修改。
2. 在“账号”中点击“接入 OpenAI 账号”，设置名称、备注、独立代理与网关限速策略。
3. 打开系统生成的 OpenAI 授权地址并完成授权。
4. 授权后地址栏会跳转到 `http://localhost:1455/auth/callback?...`。即使本地页面无法打开，也可复制完整地址并粘贴回 ShareSub 的“回调地址”输入框，然后完成接入。
5. 房主选择已接入账号创建共享方案，并选择固定分配或共享使用；固定分配模式还需设置自己的初始份额。
...
9. 用户可在默认仪表盘查看自己的今日/累计 Token、性能和最近 24 小时趋势；Plan 的所有成员还可以查看所绑定账号的完整配置、账号与成员的 5h/7d 消耗、24 小时性能和最近 7 天成员用量排行。
...
网关会按 Key 的配置从仍然有效的 Plan 中选路：`priority` 按数字从小到大选择，当前路由在请求发出前不可用或达到账号并发/RPM 上限时尝试下一条；`balanced` 在固定分配模式按成员消耗占个人份额的比例选择，在共享模式按账号总额度使用比例选择。共享 Plan 不执行个人额度限制，但账号任一有效窗口达到 100% 后会停止路由。账号独立代理优先于 `SHARESUB_OUTBOUND_PROXY`，代理请求失败时不会回退直连。只有上游明确以 `429` 或 `529` 拒绝执行时，网关才会切换到下一个账号，最多切换 3 次；连接错误、超时和其他状态不会重试，避免重复执行。系统不把请求或响应正文写入数据库或性能记录。
