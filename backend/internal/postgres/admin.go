...
func (s *Store) AdminListAccounts(ctx context.Context) ([]domain.AdminAccount, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT a.id,a.owner_user_id,a.name,a.notes,a.email,a.chatgpt_account_id,a.plan_type,a.max_concurrency,a.rpm_limit,a.fast_policy,a.token_expires_at,a.status,a.last_error,a.created_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.OwnerUserID, &item.Name, &item.Notes, &item.Email, &item.ChatGPTAccountID, &item.PlanType, &item.MaxConcurrency, &item.RPMLimit, &item.FastPolicy, &item.TokenExpiresAt, &item.Status, &item.LastError, &item.CreatedAt, &item.OwnerUsername, &item.OwnerEmail, &item.PlanID, &item.PlanName); err != nil {
			return nil, err
		}
		out = append(out, item)
	}
